...
public class SubdomainEntry extends PanelEntry {
    ...
    private final boolean showBuff; // Added showBuff field
    ...
    public SubdomainEntry(Screen parent, int width, int height, String name, ItemStack icon, String buff, boolean locked) {
        this(parent, width, height, name, icon, buff, locked, true); // Default to showing buff
    }

    public SubdomainEntry(Screen parent, int width, int height, String name, ItemStack icon, String buff, boolean locked, boolean showBuff) {
        super(width, height);
        this.parent = parent;
        this.name = name;
        this.icon = icon;
        this.buff = buff;
        this.locked = locked;
        this.showBuff = showBuff; // Store the showBuff value
    }
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        // Adjust layout based on whether buff is shown
        if (showBuff) {
            // Original layout with name at top and buff below
            ...
            // Centered layout when buff is hidden
            int nameColor = locked ? 0x888888 : 0xFFFFFF;
            graphics.drawString(Minecraft.getInstance().font, name, x + 30, y + (height - 8) / 2, nameColor);
        }
